...
import { Search, Settings, Bell, User, Plus, Cloud, FilePlus, FolderPlus, Sparkles, HelpCircle, Menu } from 'lucide-react';
...
const Navbar = ({ setSearchQuery, isSettingsOpen, setIsSettingsOpen, onLogout, onToggleSidebar }) => {
    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full px-4 md:px-8 py-3 md:py-4 z-50 flex items-center justify-between pointer-events-none bg-[#195bac] border-b border-white/10 shadow-lg transition-all duration-300">
                {/* Brand - Left */}
                <div className="pointer-events-auto flex items-center gap-2 md:gap-3 w-auto md:w-[260px] flex-shrink-0">
                    <button
                ...
                {/* Search Bar - Center */}
                <div className="pointer-events-auto flex-1 max-w-2xl px-2 md:px-8 flex items-center gap-2 md:gap-3">
                    <div className="relative group flex-1">
                        <div className="absolute inset-y-0 left-3 md:left-4 flex items-center pointer-events-none">
                            <Search className="text-blue-200 group-focus-within:text-white transition-colors" size={18} />
                        </div>
                        <input
                            type="text"
                            placeholder={search.placeholder}
                            onChange={(e) => setSearchQuery && setSearchQuery(e.target.value)}
                            className="w-full h-10 md:h-12 pl-10 md:pl-12 pr-4 bg-white/10 border border-white/20 rounded-xl md:rounded-2xl shadow-sm focus:bg-white/20 focus:shadow-md focus:border-white/40 outline-none transition-all duration-300 text-white placeholder:text-blue-200/70 font-medium text-sm md:text-base"
                        />
                    </div>
                ...
                <div className="pointer-events-auto flex items-center gap-2 md:gap-4 flex-shrink-0">
                    ...
                    <div className="relative pointer-events-auto" ref={supportRef}>
                        <CircularButton
                            ...
                            size="sm"
                            className={`bg-white/10 hover:bg-white/20 border-white/10 text-white md:w-10 md:h-10 w-9 h-9`}
                        >
                            <HelpCircle size={20} />
                        </CircularButton>
                        <DropdownMenu
                    ...
                    <div className="relative" ref={profileRef}>
                        <button
                            ...
                            className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 text-white flex items-center justify-center font-bold shadow-lg ring-2 ring-white/20 hover:ring-white/40 transition-all text-xs md:text-sm"
                        >
                            JS
                        </button>
                        <DropdownMenu
